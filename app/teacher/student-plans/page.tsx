...
import { SURAHS, calculateTotalDays, calculateTotalPages, getAdjustedPlanPreviewRange, getContiguousCompletedJuzRange, getJuzBounds, getJuzNumbersForPageRange, getNextAyahReference, getPendingMasteryJuzs, getPlanMemorizedRange, getPlanMemorizedRanges, getPreviousMemorizationBoundary, hasScatteredCompletedJuzs, getPageFloatForAyah, getSurahJuzNumbers, normalizePreviousMemorizationRanges, resolvePlanTotalDays, resolvePlanTotalPages, subtractMemorizedRangeFromRanges, type PreviousMemorizationRange } from "@/lib/quran-data"
...
export default function TeacherStudentPlansPage() {
  ...
  const [weeklyReviewMinDailyPages, setWeeklyReviewMinDailyPages] = useState("10")
  const [weeklyReviewStartDay, setWeeklyReviewStartDay] = useState("0")
  const [weeklyReviewEndDay, setWeeklyReviewEndDay] = useState("4")
  const [draftWeeklyReviewMinDailyPages, setDraftWeeklyReviewMinDailyPages] = useState("10")
  const [draftWeeklyReviewStartDay, setDraftWeeklyReviewStartDay] = useState("0")
  const [draftWeeklyReviewEndDay, setDraftWeeklyReviewEndDay] = useState("4")
  ...
  const fetchPlansForStudents = async (studentList: Student[]) => {
    ...
    try {
      const ids = studentList.map((student) => student.id).join(",")
  ...
  const handleSavePlan = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setStudentPlans((prev) => ({ ...prev, [selectedStudent.id]: refreshedData.plan || data.plan }))
  ...
  const handleDeleteMemorizedPortion = async (student: Student, range: PreviousMemorizationRange) => {
    ...
    try {
      ...
      const updatedStudent = normalizeStudentMastery({
        ...student,
        ...data.student,
      })
  ...
  const getPreviousRangeStartVerseOptions = (range: PreviousRangeDraft) => {
    ...
    const selectedSurah = SURAHS.find((surah) => surah.number === parseInt(range.startSurah, 10))
    if (!selectedSurah) return []
  ...
  const getPreviousRangeEndVerseOptions = (range: PreviousRangeDraft) => {
    ...
    const selectedSurah = SURAHS.find((surah) => surah.number === parseInt(range.endSurah, 10))
    if (!selectedSurah) return []
    ...
    <div className="min-h-screen flex flex-col bg-[#fafaf9]">
      ...
      <main className="flex-1 py-10 px-4">
        <div className="container mx-auto max-w-3xl space-y-6">
          <div className="border-b border-[#3453a7]/40 pb-5 flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-2">
                              <div className="flex-1 h-1.5 bg-neutral-100 rounded-full overflow-hidden">
                                <div
                                  ...
                                  style={{
                                    width: `${progress}%`,
                                    background: "linear-gradient(to right, #3453a7, #4f73d1)",
                                  }}
                                />
                              </div>
        ...
        >
                  ...
                  <div className="flex-1">
                    <Popover open={startOpen} onOpenChange={setStartOpen}>
                      <PopoverTrigger asChild>
                        <button className="w-full flex items-center justify-between px-3 h-10 rounded-xl border border-[#3453a7]/40 text-sm bg-white text-right hover:border-[#3453a7] transition-colors">
                        ...
                        <Command className="overflow-visible">
                          ...
                          <CommandList className="max-h-52 overflow-y-auto surah-scroll" onWheel={(e) => { e.stopPropagation(); e.currentTarget.scrollTop += e.deltaY }}>
                            {startSurahOptions.map((surah) => (
                              ...
                              >
                                {surah.name}
                                {startSurah === String(surah.number) && <Check className="w-3.5 h-3.5 text-[#3453a7]" />}
                              </CommandItem>
                            ))}
                          </CommandList>
                        </Command>
                        ...
                        <Command className="overflow-visible">
                          ...
                          <CommandList className="max-h-52 overflow-y-auto surah-scroll" onWheel={(e) => { e.stopPropagation(); e.currentTarget.scrollTop += e.deltaY }}>
                            {endSurahOptions.map((surah) => (
                              ...
                              >
                                {surah.name}
                                {endSurah === String(surah.number) && <Check className="w-3.5 h-3.5 text-[#3453a7]" />}
                              </CommandItem>
                            ))}
                          </CommandList>
                        </Command>
            ...
            <div className="mb-2 grid grid-cols-2 gap-3 pb-2">
              <div className="col-span-2 min-w-0 space-y-1.5 flex flex-col w-full">
                ...
                <Select value={dailyPages} onValueChange={setDailyPages}>
                  <SelectTrigger className="border-[#3453a7]/40 focus:border-[#3453a7] rounded-xl text-right bg-white text-sm [&>span]:truncate" dir="rtl">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="min-w-0 space-y-1.5">
                ...
                <Select value={muraajaaMode === "weekly_distributed" ? "weekly" : muraajaaPages} onValueChange={handleMuraajaaChange} dir="rtl">
                  <SelectTrigger className="border-[#3453a7]/40 focus:border-[#3453a7] rounded-xl text-right bg-white text-sm [&>span]:truncate" dir="rtl">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent dir="rtl">
                ...
                <div className="rounded-xl bg-[#3453a7]/8 border border-[#3453a7]/30 p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="text-center">
                        <p className="text-2xl font-black text-[#1a2332]">{formattedPreviewTotal}</p>
          ...
          <div className="shrink-0 border-t border-[#3453a7]/25 bg-white px-6 py-4 flex gap-3">
            <Button
              ...
              className="flex-1 border-[#3453a7]/40 text-neutral-600 rounded-xl h-10"
            >
